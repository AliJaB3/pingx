...
async def topup_ask_amount(cb: CallbackQuery, state: FSMContext):
    ...
    msg = (
        "<b>Top up wallet</b>\n\n"
        "Send the amount you transferred. Then attach receipt photos (optional).\n\n"
        f"<b>Card:</b> <code>{CARD_NUMBER}</code>\n"
        "Example: 150000\n\n"
        f"Max photos: {MAX_RECEIPT_PHOTOS}. Max per photo: {MAX_RECEIPT_MB} MB."
    )
...
async def topup_got_amount(m: Message, state: FSMContext):
    try:
        ...
        await m.reply("Invalid amount. Send a number.")
        return
    await state.update_data(amount=amount, photos=[])
    await state.set_state(Topup.note)
    await m.reply("Now send receipt photos (optional), then send 'done'.")
...
async def collect_photo(m: Message, state: FSMContext):
    ...
    try:
        ...
        if sz > int(MAX_RECEIPT_MB) * 1024 * 1024:
            await m.reply("Photo too large.")
            return
    except Exception:
        pass
    if len(photos) >= MAX_RECEIPT_PHOTOS:
        await m.reply("Maximum number of photos reached.")
        return
    photos.append(m.photo[-1].file_id)
    await state.update_data(photos=photos)
    await m.reply(f"Photo saved ({len(photos)}/{MAX_RECEIPT_PHOTOS}).")
...
async def topup_collect(m: Message, state: FSMContext):
    ...
    if m.text and m.text.strip().lower() == "done":
        ...
        for aid in ADMIN_IDS:
            try:
                if photos:
                    await m.bot.send_message(aid, f"New payment #{pid}\nFrom <a href=\"tg://user?id={m.from_user.id}\">{htmlesc(m.from_user.full_name or m.from_user.username or str(m.from_user.id))}</a>\nAmount: {amount:,}", parse_mode=ParseMode.HTML)
                    for ph in photos:
                        await m.bot.send_photo(aid, ph, caption=f"Payment #{pid}")
                    await m.bot.send_message(aid, "Actions:", reply_markup=kb)
                else:
                    await m.bot.send_message(aid, f"New payment #{pid}\nFrom <a href=\"tg://user?id={m.from_user.id}\">{htmlesc(m.from_user.full_name or m.from_user.username or str(m.from_user.id))}</a>\nAmount: {amount:,}", reply_markup=kb, parse_mode=ParseMode.HTML)
            except Exception:
                pass
        await m.reply("Payment submitted. Wait for admin approval.")
    else:
        await state.update_data(note=m.text or "")
        await m.reply("Note saved. Send 'done' when finished.")
...
async def admin_pending(cb: CallbackQuery):
    ...
    if not pend:
        await cb.message.edit_text("درخواستی در انتظار تایید نیست.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="بازگشت ⬅️", callback_data="admin")]]))
        return
...
async def admin_pay_view(cb: CallbackQuery):
    ...
    caption = (
        f"<b>Payment #{p['id']}</b>\nUser: {user_html}\nAmount: <b>{p['amount']:,}</b>\n"
        f"Note: {htmlesc(p['note'] or '-')}\nStatus: <b>{p['status']}</b>\nAt: {p['created_at'][:19].replace('T',' ')}"
    )
    ...
    if photos:
        ...
        for ph in photos:
            await cb.bot.send_photo(cb.from_user.id, ph, caption=f"Payment #{p['id']}")
        await cb.bot.send_message(cb.from_user.id, "Actions:", reply_markup=kb(pid))
    else:
...
async def admin_pay_ok(cb: CallbackQuery):
    ...
    if not row or row["status"] != "pending":
        return await cb.answer("Already handled")
    db_update_payment_status(pid, "approved")
    db_add_wallet(row["user_id"], row["amount"])
    try:
        await cb.bot.send_message(row["user_id"], f"Your payment {row['amount']:,} was approved.")
    except Exception:
        pass
...
async def admin_pay_no(cb: CallbackQuery):
    ...
    try:
        await cb.bot.send_message(row["user_id"], f"Your payment {row['amount']:,} was rejected.")
    except Exception:
        pass
    await admin_pending(cb)
