...
async def scheduler(bot:Bot):
    ...
    while True:
        try:
            ...
            for r in rows:
                uid=r["user_id"]; pid=r["id"]
                cached=cur.execute("SELECT * FROM cache_usage WHERE purchase_id=?", (pid,)).fetchone()
                if cached:
                    used=int(cached["up"] or 0)+int(cached["down"] or 0); total=int(cached["total"] or 0)
                    if total>0:
                        pct=used/total
                        if 0.80 <= pct < 0.83:
                            try: await bot.send_message(uid, f"⚠️ نزدیک سقف حجم: {int(pct*100)}% مصرف شده.")
                            except: pass
                expiry_ms=int(r["expiry_ms"] or 0)
                if expiry_ms>0:
                    days_left=math.ceil((expiry_ms-now_ms)/1000/3600/24)
                    if days_left in (3,1):
                        try: await bot.send_message(uid, f"⏳ اشتراک شما {days_left} روز دیگر منقضی می‌شود.")
                        except: pass
